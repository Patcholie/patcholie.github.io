...
class CinematicCamera {
    ...
    setupKeyboardControls() {
        document.addEventListener('keydown', (e) => {
            ...
            switch (e.key) {
                ...
                case 's':
                    e.preventDefault();
                    if (this.currentScene < CAMERA_CONFIG.scenes.length - 1) this.goToScene(this.currentScene + 1);
                    break;
                case 'Home':
                    e.preventDefault();
                    this.goToScene(0);
    ...
    goToScene(sceneIndex, animate = true) {
        if (sceneIndex < 0 || sceneIndex >= CAMERA_CONFIG.scenes.length || sceneIndex === this.currentScene || this.isTransitioning) {
            return;
        }
    ...
    updateNavigationUI() {
        document.querySelectorAll('.nav-btn').forEach((btn, index) => btn.classList.toggle('active', index === this.currentScene));
        document.querySelectorAll('.nav-dot').forEach((dot, index) => dot.classList.toggle('active', index === this.currentScene));
    ...
    updateProgressIndicator() {
        ...
        if (progressText) progressText.textContent = String(this.currentScene + 1).padStart(2, '0');
        if (progressBar) {
    ...
    updateCamera(currentTime) {
        ...
        if (this.viewport) {
            this.viewport.style.transform = `translate3d(${-this.position.x}px, ${-this.position.y}px, ${-this.position.z}px) rotateY(${-this.rotation.y}deg)`;
        }
        if (this.cinematicBg && !isMobile) {
            this.cinematicBg.style.transform = `translate3d(${this.position.x * 0.1 + this.mouseInfluence.x}px, ${this.position.y * 0.1 + this.mouseInfluence.y}px, 0px) rotateZ(${this.rotation.y * 0.1}deg)`;
        }
    }
...
class ParticleUniverse {
    ...
    createCodeFragments() {
        ...
        if (!container) return;
        const snippets = ['std::mutex', 'class HitchHiker', 'transmit()', 'secure', 'pthread_t', 'encrypt()', '0x41414141', 'TCP_NODELAY', 'socket()', 'bind()', 'listen()', 'accept()', 'recv()', 'send()', 'close()'];
...
class InteractiveEffects {
    ...
    // *** NEW: Method to handle custom cursor and hover effects
    setupCursor() {
        ...
        const interactiveElements = document.querySelectorAll('a, button, .project-cube, .skill-planet, .timeline-node, .dot');
